...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    var medusa = new Medusa('Medussa');
    var victim1 = new Person('Hugo Strange');
    var victim2 = new Person('Joker');
    var victim3 = new Person('Catwoman');
    var victim4 = new Person('Batman');

    medusa.stare(victim1);
    assert.equal(medusa.statues[0].name, "Hugo Strange");
    medusa.stare(victim2);
    medusa.stare(victim3);
    medusa.stare(victim4);

    assert.equal(medusa.statues[2].name, "Batman");
    assert.equal(medusa.statues[0].name, "Joker")
  });
})
